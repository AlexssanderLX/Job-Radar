...
export function HistoryPage() {
  ...
  return <div className="space-y-4 max-w-5xl"><PageHeader title="Histórico" description={`${items.length} pesquisas · ${total} resultados`}/><div className="overflow-hidden rounded-lg border border-zinc-800">{items.map((item) => <div key={item.id} className="flex items-center gap-3 border-b border-zinc-800 px-4 py-3 last:border-0"><Search size={14} className="text-zinc-500"/><div className="flex-1"><p className="text-sm text-zinc-200">{item.total_found} vagas encontradas</p><p className="text-xs text-zinc-500">{new Date(item.searched_at).toLocaleString('pt-BR')} · {item.duration_seconds.toFixed(1)}s</p></div>{item.sources_failed.length > 0 && <span className="text-xs text-amber-400">{item.sources_failed.length} falha(s)</span>}</div>)}{!items.length && <EmptyState title="Sem histórico" description="As pesquisas executadas aparecerão aqui."/ >}</div></div>
}
